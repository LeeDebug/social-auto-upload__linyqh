...
async def cookie_auth(account_file):
    async with async_playwright() as playwright:
        ...
        try:
            ...
            weibo_logger.info("[+] 等待5秒 cookie 失效")
            return False
        except:
            weibo_logger.success("[+] cookie 有效")
            return True
...
async def weibo_setup(account_file, handle=False):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        if not handle:
            return False
        weibo_logger.info('[+] cookie文件不存在或已失效，即将自动打开浏览器，请扫码登录，登陆后会自动生成cookie文件')
...
async def get_weibo_cookie(account_file):
    async with async_playwright() as playwright:
        ...
        weibo_logger.success("[+] account 文件写入成功")
        weibo_logger.info("[+] 如需检测 cookie 是否有效，请重新运行该脚本")
...
class WeiBoVideo(object):
    ...
    async def handle_upload_error(self, page):
        weibo_logger.error("视频出错了，重新上传中")
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        weibo_logger.info('正在上传-------{}.mp4'.format(self.title))
        # 等待页面跳转到指定的 URL，没进入，则自动等待到超时
        weibo_logger.info('正在打开主页...')
        ...
        while retry_count < max_retries:
            try:
                ...
                print("上传中 number: ", number)
                number1 = await page.locator("text=上传完成").count()
                print("上传完成 number1: ", number1)

                if number == 0:
                    weibo_logger.success("视频上传完毕")
                    break
                else:
                    if retry_count % 5 == 0:
                        weibo_logger.info("正在上传视频中...")
                    await asyncio.sleep(2)
            except Exception as e:
                weibo_logger.error(f"检查上传状态时发生错误: {e}")
                await asyncio.sleep(2)  # 等待 2 秒后重试
            retry_count += 1

        if retry_count == max_retries:
            weibo_logger.warning("超过最大重试次数，视频上传可能未完成。")
        ...
        while True:
            try:
                ...
                weibo_logger.success("视频发布成功")
                break
            except Exception as e:
                weibo_logger.info(f"视频正在发布中... 错误: {e}")
                await page.screenshot(full_page=True)
                await asyncio.sleep(1)

        await context.storage_state(path=self.account_file)  # 保存cookie
        weibo_logger.info('cookie更新完毕！')
    ...
    async def set_schedule_time(self, page, publish_date):
        weibo_logger.info("click schedule")
