...
class TikTokUploaderGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TikTok视频上传工具")
    ...
    def populate_account_files(self):
        self.log("加载账户配置文件...")
        """填充账户配置文件下拉框"""
        account_dir = Path(BASE_DIR) / "cookies" / "tk_uploader"
        ...
        try:
            ...
            for index, file in enumerate(files):
                ...
                if thumbnail_path.exists():
                    self.log(f"缩略图文件名：{thumbnail_path}")
