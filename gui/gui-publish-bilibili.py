...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QTextEdit, QWidget, QSpinBox, QCheckBox
)
...
class BilibiliUploaderGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("B站视频上传工具")
    ...
    def init_ui(self):
        ...
        self.daily_times_input = QLineEdit("14")
        ...
        self.days_ahead_input = QSpinBox()
        self.days_ahead_input.setValue(1)
    ...
    def populate_account_files(self):
        self.log("加载账户配置文件...")
        """填充账户配置文件下拉框"""
        account_dir = Path(BASE_DIR) / "cookies" / "bilibili_uploader"
    ...
    def start_upload(self):
        ...
        try:
            ...
            if is_scheduled:
                ...
                days_ahead = self.days_ahead_input.value()
                timestamps = generate_schedule_time_any_day(len(files), days_ahead, daily_times, start_date,
                                                            timestamps=True)
                self.log(f"生成的时间戳: {timestamps}")
            else:
            ...
            for index, file in enumerate(files):
                ...
                desc = title
                self.log(f"视频标题: {title}, 标签: {tags_str}")

                bili_uploader = BilibiliUploader(cookie_data, file, title, desc, VideoZoneTypes.SPORTS_FOOTBALL.value,
                                                 [tags_str], timestamps[index])
                self.log(f"开始上传视频: {file.name}...")
