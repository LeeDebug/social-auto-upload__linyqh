...
class TencentUploaderGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("腾讯视频上传工具")
    ...
    def init_ui(self):
        ...
        self.log("创建选择视频分类下拉框...")
        # 选择视频分类（这里假设使用腾讯视频相关分类，对应原代码中的category设置）
        self.layout.addWidget(QLabel("选择视频分类:"))
    ...
    def populate_account_files(self):
        self.log("加载账户配置文件...")
        """填充账户配置文件下拉框"""
        account_dir = Path(BASE_DIR) / "cookies" / "tencent_uploader"
    ...
    def populate_category_options(self):
        self.log("加载视频分类选项...")
        """填充视频分类下拉框，这里简单示例从TencentZoneTypes获取分类名称"""
        category_names = [zone_type.value for zone_type in TencentZoneTypes]
        self.category_combo.addItems(category_names)
        self.log(f"找到以下视频分类: {'; '.join(category_names)}")
